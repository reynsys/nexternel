# Nexternel V3.1.1 — Development Bible

Permanent planning reference for Generation 3. **No implementation until documents are reviewed and approved.**

| # | Document | Path |
|---|----------|------|
| 0 | **Master Architecture Plan** | [00-MASTER-ARCHITECTURE-PLAN.md](00-MASTER-ARCHITECTURE-PLAN.md) |
| 1 | Vision | [01-VISION.md](01-VISION.md) |
| 2 | Competitive Analysis & Differentiation | [02-COMPETITIVE-ANALYSIS.md](02-COMPETITIVE-ANALYSIS.md) |
| 3 | Product Requirements (PRD) | [03-PRD.md](03-PRD.md) |
| 4 | Software Architecture Specification | [04-SOFTWARE-ARCHITECTURE.md](04-SOFTWARE-ARCHITECTURE.md) |
| 5 | Implementation & Migration Plan | [05-IMPLEMENTATION-MIGRATION-PLAN.md](05-IMPLEMENTATION-MIGRATION-PLAN.md) |
| 6 | Phase 1 Foundation notes | [06-PHASE1-FOUNDATION.md](06-PHASE1-FOUNDATION.md) |
| 7 | Phase 2 Backend API notes | [07-PHASE2-BACKEND-API.md](07-PHASE2-BACKEND-API.md) |
| 8 | Phase 3/4 Telemetry + Capabilities | [08-PHASE3-4-TELEMETRY-CAPABILITIES.md](08-PHASE3-4-TELEMETRY-CAPABILITIES.md) |
| 9 | Phase 5/6 Dashboards | [09-PHASE5-6-DASHBOARDS.md](09-PHASE5-6-DASHBOARDS.md) |
| 10 | Phase 7 History & Charts | [10-PHASE7-HISTORY.md](10-PHASE7-HISTORY.md) |
| 11 | Phase 8 Admin + Plugins | [11-PHASE8-ADMIN-PLUGINS.md](11-PHASE8-ADMIN-PLUGINS.md) |
| 12 | Phase 9 Cutover (parallel run) | [12-PHASE9-CUTOVER.md](12-PHASE9-CUTOVER.md) |
| 13 | Phase 10 Retire Next.js | [13-PHASE10-RETIRE-NEXT.md](13-PHASE10-RETIRE-NEXT.md) |
| 14 | UI skins / templates | [14-UI-SKINS.md](14-UI-SKINS.md) |
| 15 | Dashboard sections | [15-DASHBOARD-SECTIONS.md](15-DASHBOARD-SECTIONS.md) |
| 16 | Widget catalog categories | [16-WIDGET-CATALOG.md](16-WIDGET-CATALOG.md) |
| 17 | ECharts widgets | [17-ECHARTS-WIDGETS.md](17-ECHARTS-WIDGETS.md) |
| 18 | Areas | [18-AREAS.md](18-AREAS.md) |
| 19 | Devices | [19-DEVICES.md](19-DEVICES.md) |
| 20 | Live switch idle | [20-LIVE-SWITCH-IDLE.md](20-LIVE-SWITCH-IDLE.md) |
| 21 | General dashboard widgets | [21-GENERAL-WIDGETS.md](21-GENERAL-WIDGETS.md) |
| 22 | Dashboard tabs | [22-DASHBOARD-TABS.md](22-DASHBOARD-TABS.md) |
| 23 | Roles | [23-ROLES.md](23-ROLES.md) |

**Baseline:** V2.1.207 → **Target:** V3.1.1 (acceptance)  

**Status:** Architecture approved · Phases 1–10 shipped (operator UI `:8080` only; V2 Next.js removed)
