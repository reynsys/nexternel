...
1. Edit `VERSION_SOFTWARE` (or `VERSION_HARDWARE` / `VERSION_GENERATION`) in `apps/web/src/lib/version.ts`.
2. Add a new `## VX.Y.Z — DD/MM/YYYY` section **above** the previous release.
3. Group changes under headings if helpful (e.g. Dashboard, Admin, Infrastructure).
4. List **files added** when introducing new modules.
